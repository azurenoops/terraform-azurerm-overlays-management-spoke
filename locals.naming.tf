locals {
  ...
  spoke_vnet_name        = coalesce(var.custom_virtual_network_name, data.azurenoopsutils_resource_name.vnet.result)  
  spoke_rt_name          = coalesce(var.custom_route_table_name, "${data.azurenoopsutils_resource_name.rt.result}")
  spoke_sa_name          = coalesce(var.custom_storage_account_name, data.azurenoopsutils_resource_name.st.result)
